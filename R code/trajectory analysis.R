...
deaths <- owid %>%
  ...
  mutate(
    ...
    total_death_rate=weight_death*1000000*total_deaths/population,
    ...
    death_rate_smooth = weight_death*1000000*deaths_smooth/population,
...
r3$clust.distr
death_rate_clusters %>% filter(cluster !=3) %>% select(cluster,ISO_country)
...
plotMeanTraj(r3, ylim=c(0,8))
plotMedTraj(r3, ylim=c(0,8))
plotBoxplotTraj(r3)
plotCombTraj(r3)  #
plotCombTraj(r3,stat.type = "median", ylim=c(0,8), col=1:maxclust) 
...
t3$clust.distr
total_death_rate_clusters %>% filter(cluster !=1) %>% select(cluster,ISO_country)
...
plotMedTraj(t3, ylim=c(0,400))
plotBoxplotTraj(t3)
plotCombTraj(t3)  #
plotCombTraj(t3,ylim=c(0,300), col=1:maxclust) 
legend("topleft", legend=1:maxclust,
       pch=1:maxclust,lty=1:maxclust, col=1:maxclust)
